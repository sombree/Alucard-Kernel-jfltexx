...
static void alucard_check_cpu(struct cpufreq_alucard_cpuinfo *this_alucard_cpuinfo)
{
	...
	if (cur_load >= inc_cpu_load && index < this_alucard_cpuinfo->max_index) {
				...
				unsigned int event)
{
	...
	case CPUFREQ_GOV_START:
		...
		this_alucard_cpuinfo->freq_table = cpufreq_frequency_get_table(this_alucard_cpuinfo->cpu);
		...
		this_alucard_cpuinfo->prev_cpu_idle = get_cpu_idle_time(this_alucard_cpuinfo->cpu,
